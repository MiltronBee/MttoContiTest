...
export const SolicitarSuplente = () => {
    ...
    // Areas state for roles that require AreaId
    const [areasLoading, setAreasLoading] = useState(false);
    const [areaOptions, setAreaOptions] = useState<Array<{ id: number; nombre: string }>>([]);
    const [ownedAreas, setOwnedAreas] = useState<Area[]>([]); // Solo para Area Admin
    const [engineerAreas, setEngineerAreas] = useState<AreaByIngenieroItem[]>([]); // Solo para Industrial
    const [selectedAreaId, setSelectedAreaId] = useState<number | undefined>(undefined);

    // Cargar candidatos de suplente por rol del usuario
    useEffect(() => {
    ...
    // Cargar áreas según rol cuando apliquen (Industrial / Area Admin)
    useEffect(() => {
    ...
    // Helper: obtener AreaId según el rol cuando no viene en el usuario
    const resolveAreaIdForRole = async (role: string): Promise<number | undefined> => {
        if (!user?.id) return undefined;

        try {
            if (role === UserRole.INDUSTRIAL) {
                // Obtener áreas asignadas al ingeniero y usar la primera activa
                const res = await areasService.getAreasByIngeniero(user.id);
            ...
            if (role === UserRole.AREA_ADMIN) {
                // Traer todas las áreas y filtrar por jefeId === user.id
    ...
    // Helper: construir payload para API según rol del usuario
    const buildPayload = async (suplenteId: number | null | undefined) => {
        ...
        if (role === UserRole.INDUSTRIAL) {
            // Si hay selector y el usuario escogió un área, usarla
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await userService.requestSuplente(payload);
            toast.success(`Solicitud de suplente enviada exitosamente`);

            // Refrescar área seleccionada para ver suplente actualizado (solo Area Admin)
            ...
            // Refrescar listado de áreas del ingeniero (Industrial)
    ...
    const handleRemove = async () => {
        try {
            ...
            // Refrescar área seleccionada (Area Admin)
            if (payload.AreaId && getUserRole(user) === UserRole.AREA_ADMIN) {
                try {
                    const updated = await areasService.getAreaById(payload.AreaId);
                    setOwnedAreas(prev => {
                                ...
                                <AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}>
                                    <AlertDialogTrigger asChild>
                                        <Button
                                            type="button"
                                            variant="outline"
                                            className="px-6 py-2 border-continental-red text-continental-red"
                                            disabled={removing}
                                        >
