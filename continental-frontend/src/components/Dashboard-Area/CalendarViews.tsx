...
export const DailyView: React.FC<ViewProps> = ({ calendarData, currentDate, selectedGroups, currentAreaGroups, ausenciasData }) => {
    ...
    const getPersonalData = (groupId: string) => {
        ...
        if (grupoAusencias) {
            return {
                disponible: [], // TODO: El API no devuelve lista de disponibles, solo el count
                noDisponible: grupoAusencias.empleadosAusentes.map(emp => ({
                    id: emp.empleadoId.toString(),
                    nombre: emp.nombreCompleto,
                    motivo: emp.tipoAusencia + (emp.tipoVacacion ? ` (${emp.tipoVacacion})` : '') + (emp.maquina ? ` - ${emp.maquina}` : '')
                })),
                stats: {
                                ...
                                <div className="grid md:grid-cols-2 gap-6 p-6">
                                    <div>
                                        ...
                                        <div className="text-center text-gray-500 py-4">
                                            <p className="text-sm">Lista detallada no disponible</p>
                                            <p className="text-xs text-gray-400 mt-1">
                                                {stats ? `${stats.personalDisponible} empleados disponibles` : 'Sin datos'}
                                            </p>
                                        </div>
                                    </div>
