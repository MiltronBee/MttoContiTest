...
export const Register = ({ onCancel, onSuccess }: RegisterProps) => {
  ...
  const handleInputChange = (field: string, value: string) => {
    setFormData(prev => {
      if (field === 'roles') {
        // Handle multiple role selection - store role IDs as strings
        const roleId = value; // value is already the role ID from the select
        
        const currentRoles = prev.roles || [];
        const updatedRoles = currentRoles.includes(roleId)
          ? currentRoles.filter(role => role !== roleId) // Remove if already selected
          : [...currentRoles, roleId]; // Add if not selected
                    ...
                    return (
                      ...
                      >
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            handleInputChange('roles', roleId);
                          }}
