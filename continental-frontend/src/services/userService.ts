...
export const userService = {
  ...
  async requestSuplente(payload: { Rol: string; GrupoId?: number; AreaId?: number; SuplenteId?: number | null }): Promise<{ success: boolean }>{
    try {
      ...
      if (response?.success === false) {
        console.error("Error en solicitud de suplente:", response);
        throw new Error(response?.message || response?.errorMsg || "Error en solicitud de suplente");
      }
