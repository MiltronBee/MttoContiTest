...
class AuthService {
  ...
  async login(credentials: LoginRequest): Promise<LoginResponse & { user: User }> {
    try {
      ...
      logger.authAction('Login attempt', { username: identifier });
      ...
      // Guardar el token y expiration temporalmente para que httpClient pueda acceder a él
      localStorage.setItem(this.TOKEN_KEY, token);
      localStorage.setItem(this.EXPIRATION_KEY, expiration);
      ...
      const user: User = {
        ...
        roles: userData?.roles || 
               (userData?.rols && Array.isArray(userData?.rols) && userData?.rols.length > 0 && typeof userData?.rols[0] === 'object' 
                ? (userData?.rols as Rol[]).map((rol: Rol) => rol.name)
                : []),
